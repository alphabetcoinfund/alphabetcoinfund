    ...
    rpcConsole(0)
{
  ...
  //connect(mainPage->sideBar, SIGNAL(clickedLogoImage()), this, SLOT(aboutClicked()));
    ...
    setWindowTitle(tr("AlphabetCoinFund") + " - " + tr("Wallet"));
#ifndef Q_OS_MAC
...
//    connect(mainPage->pageTransactions->table->table, SIGNAL(doubleClicked(QModelIndex)), this, mainPage->pageTransactions, SLOT(showDetails()));
    ...
    connect(mainPage->pageReceiveFUN, SIGNAL(signMessage(QString)), this, SLOT(gotoSignMessageTab(QString)));
...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsAction->setToolTip(tr("Send coins to a AlphabetCoinFund address"));
    ...
    aboutAction = new QAction(QIcon(":/icons/bitcoin"), tr("&About AlphabetCoinFund"), this);
    aboutAction->setToolTip(tr("Show information about AlphabetCoinFund"));
    aboutAction->setMenuRole(QAction::AboutRole);
    aboutQtAction = new QAction(QIcon(":/trolltech/qmessagebox/images/qtlogo-64.png"), tr("About &Qt"), this);
    aboutQtAction->setToolTip(tr("Show information about Qt"));
    aboutQtAction->setMenuRole(QAction::AboutQtRole);
    optionsAction = new QAction(QIcon(":/icons/options"), tr("&Options..."), this);
    optionsAction->setToolTip(tr("Modify configuration options for AlphabetCoinFund"));
...
void BitcoinGUI::setClientModel(ClientModel *clientModel)
{
    ...
    if(clientModel)
    {
        ...
        if(clientModel->isTestNet())
        {
            ...
            if(trayIcon)
            {
                trayIcon->setToolTip(tr("AlphabetCoinFund client") + QString(" ") + tr("[testnet]"));
        ...
        mainPage->pageReceiveFUN->setOptionsModel(clientModel->getOptionsModel());
    }
}
...
void BitcoinGUI::createTrayIcon()
{
    ...
    trayIcon->setToolTip(tr("AlphabetCoinFund client"));
...
void BitcoinGUI::setNumConnections(int count)
{
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to AlphabetCoinFund network.", "", count));
}
...
void BitcoinGUI::gotoReceiveCoinsPage()
{
    ...
    showSelectedPageItem(SC_PAGE_ID_RECEIVE_FUN);

    exportAction->setEnabled(true);
    disconnect(exportAction, SIGNAL(triggered()), 0, 0);
    connect(exportAction, SIGNAL(triggered()), mainPage->pageReceiveFUN, SLOT(exportClicked()));
}

void BitcoinGUI::gotoSendCoinsPage()
{
    sendCoinsAction->setChecked(true);
    showSelectedPageItem(SC_PAGE_ID_SEND_FUN);
...
void BitcoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
        ...
        {
            if (mainPage->pageSendFUN->sendCoinsPage->handleURI(uri.toString()))
                nValidUrisFound++;
        }

        // if valid URIs were found
        if (nValidUrisFound)
            slotSelectedPageItem(SC_PAGE_ID_SEND_FUN);
        else
            notificator->notify(Notificator::Warning, tr("URI handling"), tr("URI can not be parsed! This can be caused by an invalid AlphabetCoinFund address or malformed URI parameters."));
    }
...
{
    // URI has to be valid
    if (mainPage->pageSendFUN->sendCoinsPage->handleURI(strURI))
    {
...
void BitcoinGUI::slotSelectedPageItem(int id)
{
    ...
    case SC_PAGE_ID_SEND_FUN:
        showSelectedPageItem(page_id);
        exportAction->setEnabled(false);
        disconnect(exportAction, SIGNAL(triggered()), 0, 0);
        break;

    case SC_PAGE_ID_RECEIVE_FUN:
        showSelectedPageItem(page_id);
        exportAction->setEnabled(true);
        disconnect(exportAction, SIGNAL(triggered()), 0, 0);
        connect(exportAction, SIGNAL(triggered()), mainPage->pageReceiveFUN, SLOT(exportClicked()));
